...
init()
{
	...
	//maps\mp\gametypes\_hardpoints::registerKillstreakAltWeapon("supply_drop_mp", "minigun_mp" );
	maps\mp\gametypes\_hardpoints::registerKillstreakAltWeapon("supply_drop_mp", "mp40_blinged_mp" );
//	maps\mp\gametypes\_hardpoints::setRemoveWeaponWhenUsed("supply_drop_mp", false);hud_ks_m202
	...
	while( IsDefined(self) )
	{
		...
		if ( player HasPerk( "specialty_fastinteract" ) )
		{
			if( level.teambased && player.team == self.owner.team )
			{
				...
				result = useEnt useHoldThink( player, level.crateNonOwnerUseTime, self);
			}
			else
			{
				result = useEnt useHoldThink( player, level.crateOwnerUseTime, self);
			}
		}
		else
		{
...
crateUseThinkOwner(crate) // self == crate
{
...
useHoldThink( player, useTime , crate ) // self == a script origin (useEnt) or the crate
{
